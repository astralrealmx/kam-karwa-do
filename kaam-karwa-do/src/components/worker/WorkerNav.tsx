...
const LINKS: { href: string; key: string; icon: string }[] = [
  { href: "/worker/dashboard", key: "dashboard", icon: "📊" },
  { href: "/worker/availability", key: "availability", icon: "🗓️" },
  { href: "/worker/profile", key: "profileTitle", icon: "👤" },
  { href: "/worker/verification", key: "verification", icon: "🛡️" },
  { href: "/worker/settings", key: "settingsLabel", icon: "⚙️" },
];

export function WorkerNav({ user }: { user: SessionUser }) {
  const { t } = useLanguage();
  ...
  function label(key: string) {
    if (key === "settingsLabel") return t("settings.title");
    return t(`worker.${key}`);
  }
    ...
    <>
      ...
      <aside className="hidden w-56 flex-shrink-0 md:block">
        <div className="mb-4 rounded-xl bg-white p-4 shadow-card">
          <p className="text-sm font-semibold text-ink">{user.name ?? "Worker"}</p>
